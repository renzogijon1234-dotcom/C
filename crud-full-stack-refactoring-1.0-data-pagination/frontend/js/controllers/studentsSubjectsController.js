...
async function loadStudentsSubjects()  //StudentsSubjects
{
    try 
    {
        ...
        console.log(data);
        loadRelationsPaginated(data.students_subjects);
...
async function loadRelationsPaginated(relations)  //modificacion para que funciona para la paginacion
{
    {
        try {
            if (!Array.isArray(relations)) {
                console.error('Error: relations no es un array', relations);
                return;
            }
            relations.forEach(rel => {
                console.log('probando');
                console.log(`rel es: ${rel.approved}`);
                rel.approved = Number(rel.approved);
            });
            renderRelationsTable(relations);
        } catch (err) {
